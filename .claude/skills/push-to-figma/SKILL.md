...
Push data to the Figma plugin. Ensure the bridge server is running (`pnpm run bridge` in figma-designer-plugin/).

**Push tokens:**
```bash
cd figma-designer-plugin && pnpm run push:tokens
```
...
```bash
cd figma-designer-plugin && pnpm run push:component -- ../components/{ComponentName}
```
