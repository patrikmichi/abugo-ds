...
import { pairVariablesWithFigma } from './pairVariables';
...
/**
 * Handle PAIR_VARIABLES message
 */
async function handlePairVariables(design: DesignJson): Promise<void> {
  try {
    console.log(`Pairing variables for: ${design.meta.name}`);
    const result = await pairVariablesWithFigma(design);
    console.log(`Pairing complete: ${result.stats.paired} paired, ${result.stats.missing} missing`);
    ...
    figma.notify(
      `${result.stats.paired} variables paired, ${result.stats.missing} missing`,
      { timeout: 3000 }
    );
  } catch (error) {
    console.error('Error pairing variables:', error);
    sendToUI({
      type: 'ERROR',
      error: error instanceof Error ? error.message : String(error),
    });
...
figma.ui.onmessage = async (msg: PluginMessage) => {
  switch (msg.type) {
    ...
    case 'PAIR_VARIABLES':
      await handlePairVariables(msg.payload);
      break;
