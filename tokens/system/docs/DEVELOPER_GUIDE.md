...
This command:
- Reads tokens from `tokens/output/primitives.json` and `tokens/output/semanticTokens.json`
...
2. **Check token name**: Verify the token exists in `tokens/output/primitives.json` or `tokens/output/semanticTokens.json`
