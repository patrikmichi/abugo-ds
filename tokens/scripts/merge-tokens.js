...
fs.writeFileSync(
  path.join(tokensDir, 'output', 'primitives.json'),
...
fs.writeFileSync(
  path.join(tokensDir, 'output', 'semanticTokens.json'),
  JSON.stringify(mergedSemantic, null, 2),
  'utf8'
);
console.log(`✅ Merged ${fileCount} semantic files → output/semanticTokens.json`);
...
fs.writeFileSync(
  path.join(tokensDir, 'output', 'componentTokens.json'),
